...
export const aiService = {
  assessRisk: async (sensorData: any) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/ai/risk-assessment`, {
        method: 'POST',
  ...
  trainModel: async (trainingData: any) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/ai/train`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const weatherService = {
  getCurrentWeather: async (lat: number = 28.6139, lon: number = 77.2090) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/weather/current?lat=${lat}&lon=${lon}`);
      return await response.json();
    } catch (error) {
  ...
  getForecast: async (lat: number = 28.6139, lon: number = 77.2090, days: number = 7) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/weather/forecast?lat=${lat}&lon=${lon}&days=${days}`);
      return await response.json();
    } catch (error) {
  ...
  analyzeImpact: async (weatherData: any, damData: any) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/weather/impact-analysis`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const iotService = {
  getAllSensors: async () => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/iot/sensors`);
      return await response.json();
    } catch (error) {
  ...
  getSensorData: async (sensorId: string) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/iot/sensor/${sensorId}`);
      return await response.json();
    } catch (error) {
  ...
  calibrateSensor: async (sensorId: string, calibrationData: any) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/iot/sensor/${sensorId}/calibrate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const analyticsService = {
  getOverview: async (startDate: string, endDate: string) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/analytics/overview?startDate=${startDate}&endDate=${endDate}`);
      return await response.json();
    } catch (error) {
  ...
  getWaterFlow: async (days: number = 30) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/analytics/water-flow?days=${days}`);
      return await response.json();
    } catch (error) {
  ...
  generateReport: async (reportType: string, parameters: any) => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/analytics/generate-report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const visualization3DService = {
  getStructureData: async () => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/3d/structure-data`);
      return await response.json();
    } catch (error) {
  ...
  getStressPoints: async () => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/3d/stress-points`);
      return await response.json();
    } catch (error) {
