...
const Alerts = () => {
  const { t } = useLanguage();
  const { selectedDam, getDamLabel, getDamLocation } = useDam();
  ...
  useEffect(() => {
    setAlerts(prev => {
      const updated = [...prev];
      if (updated[0]) {
        updated[0].dam = getDamLabel(selectedDam);
        updated[0].location = getDamLocation(selectedDam) ? `${getDamLocation(selectedDam)[0].toFixed(2)}, ${getDamLocation(selectedDam)[1].toFixed(2)}` : 'Unknown';
      }
      return updated;
    });
  }, [selectedDam, getDamLabel, getDamLocation]);
  ...
  const [alerts, setAlerts] = useState<Alert[]>([
    {
      ...
      location: getDamLocation(selectedDam) ? `${getDamLocation(selectedDam)[0].toFixed(2)}, ${getDamLocation(selectedDam)[1].toFixed(2)}` : 'Unknown',
      type: 'Seismic Activity',
      description: `Seismic activity detected - ${(selectedDam === 'sardar' ? 0.9 : 0.5)} Richter scale. Immediate monitoring required.`,
