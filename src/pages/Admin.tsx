...
import { Download, Users, AlertTriangle, RefreshCw, CheckCircle, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingAccounts.length > 0 && (
        <div className="glass-card rounded-2xl p-6 border-yellow-500/30">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Users className="w-5 h-5 text-yellow-500" />
              <h2 className="text-xl font-bold text-foreground">Pending Approvals</h2>
              <span className="text-sm text-muted-foreground">({pendingAccounts.length})</span>
            </div>
            <button
              onClick={fetchPendingAccounts}
              disabled={loading}
              className="inline-flex items-center gap-2 px-3 py-2 rounded-md bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30 text-sm"
            >
              ...
              <tbody>
                {pendingAccounts.map((account, idx) => (
                  <tr key={idx} className="border-b border-primary/10 hover:bg-primary/5">
                    <td className="py-3 px-2 text-sm">{account.username}</td>
                    ...
                    <td className="py-3 px-2 text-sm flex gap-2">
                      <button 
                        onClick={() => handleStatusUpdate(account._id, 'approved')}
                        disabled={loading}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-green-500/20 text-green-500 hover:bg-green-500/30 text-xs"
                      >
                      ...
                      <button 
                        ...
                        disabled={loading}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-red-500/20 text-red-500 hover:bg-red-500/30 text-xs"
                      >
