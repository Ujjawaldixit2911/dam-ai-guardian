...
// Helper functions to generate realistic map shapes
const generateBlobPolygon = (center: [number, number], radiusInMeters: number, irregularity = 0.25, seedOffset = 0) => {
  const points = 60; // High resolution
  const coords: [number, number][] = [];
  const radiusInDegrees = radiusInMeters / 111000;
  
  // Combine location and a specific offset (like radius) so each zone layer has a distinctly different shape
  const uniqueFactor = (center[0] + center[1]) * 10 + seedOffset;
  
  for (let i = 0; i < points; i++) {
    const angle = (i / points) * (2 * Math.PI);
    
    // Complex wave combination to create very organic, non-circular shapes
    const seed = Math.sin(angle * 3 + uniqueFactor) * 0.6 + Math.cos(angle * 2 - uniqueFactor) * 0.4;
    const r = radiusInDegrees * (1 + irregularity * seed);
    
    // Unique directional stretch per zone
    const stretchLat = 1.0 + Math.abs(Math.sin(uniqueFactor)) * 0.4;
    const stretchLng = 1.0 + Math.abs(Math.cos(uniqueFactor)) * 0.4;
    
    const lat = center[0] + r * Math.cos(angle) * stretchLat;
    const lng = center[1] + r * Math.sin(angle) * stretchLng;
    coords.push([lat, lng]);
  }
  return coords;
};

const makeRouteWavy = (coords: [number, number][], routeId = '1') => {
  if (coords.length < 2) return coords;
  const start = coords[0];
  const end = coords[coords.length - 1];
  const newCoords: [number, number][] = [start];
  
  const segments = 20; // More segments for a curvier road
  const dx = end[0] - start[0];
  const dy = end[1] - start[1];
  const seed = parseInt(routeId) || 1;
  
  for (let i = 1; i < segments; i++) {
    const t = i / segments;
    let lat = start[0] + dx * t;
    let lng = start[1] + dy * t;
    
    // Add distinct curves imitating mountain roads
    const wave1 = Math.sin(t * Math.PI * 6 + seed) * 0.004;
    const wave2 = Math.cos(t * Math.PI * 3 - seed) * 0.002;
...
function MapLayers({ 
  ...
  return (
    <>
      {/* Flood Zones */}
      {showFloodZones && floodZones.map((zone: any, index: number) => (
        <Polygon
          key={zone.id}
          // Pass index to ensure each concentric ring has a drastically different shape!
          positions={generateBlobPolygon(zone.center, zone.radius, 0.25, index * 100)}
          pathOptions={{
          ...
          <Popup>
            <div className="min-w-[200px]">
              ...
              <div className="mt-2 space-y-1">
                <p className="text-sm font-medium">Risk Level: <span style={{ color: riskColors[zone.risk] }}>{zone.risk.toUpperCase()}</span></p>
                <p className="text-sm">Impact Area: ~{(zone.radius / 1000).toFixed(1)} km radius</p>
                <p className="text-sm">Vulnerable Population: {zone.population.toLocaleString()}</p>
              </div>
            </div>
          </Popup>
        </Polygon>
      ...
      {showEvacuationRoutes && evacuationRoutes.map((route: any) => (
        <Polyline
          key={route.id}
          positions={makeRouteWavy(route.coordinates, route.id)}
          pathOptions={{
            color: routeColors[route.status],
            weight: 5,
            opacity: 0.9,
            dashArray: route.status === 'blocked' ? '10, 15' : undefined,
            lineCap: 'round',
            lineJoin: 'round'
          }}
        >
          <Popup>
            <div className="min-w-[200px]">
              <strong className="text-lg">🛣️ {route.name}</strong>
              <div className="mt-2 space-y-1">
                <p className="text-sm font-medium">Status: <span style={{ color: routeColors[route.status] }}>{route.status.toUpperCase()}</span></p>
                <p className="text-sm">Evacuation Capacity: {route.capacity.toLocaleString()} people/hr</p>
              </div>
            </div>
          </Popup>
        </Polyline>
      ...
      {showSafeLocations && safeLocations.map((location: any, idx: number) => {
        ...
        // Presentable realistic markers for safe zones
        const customIcon = L.divIcon({
          className: 'custom-safe-zone-marker',
          html: `<div style="background-color: #22c55e; color: white; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 3px solid white; box-shadow: 0 0 15px rgba(34, 197, 94, 0.8); font-size: 16px;">⛑️</div>`,
          iconSize: [32, 32],
          iconAnchor: [16, 16]
        });
        ...
        return (
          ...
          >
            <Popup>
              <div className="min-w-[200px]">
                <strong className="text-lg text-green-600">⛑️ {location.name}</strong>
                <div className="mt-2 space-y-1">
                  <p className="text-sm font-medium">Relief Capacity: {location.capacity.toLocaleString()} beds</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    Coordinates: {pos[0].toFixed(4)}, {pos[1].toFixed(4)}
                  </p>
                  <p className="text-xs text-green-600 mt-1">● Active & Stocked</p>
                </div>
              </div>
            </Popup>
          </Marker>
...
const GISMapping = () => {
  const { t } = useLanguage();
  const { selectedDam, setSelectedDam, getDamLocation, getDamLabel, dams } = useDam();
      ...
      <Card className="p-6 glass-card">
        ...
        <div className="rounded-lg overflow-hidden border border-border" style={{ height: '600px' }}>
          ...
          >
            ...
            <MapUpdater center={damLocation} trigger={recenterTrigger} />
          </MapContainer>
        </div>
