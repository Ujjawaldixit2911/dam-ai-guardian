...
if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
  try {
    ...
    console.error('Failed to initialize Twilio (Check if Account SID starts with AC):', error.message);
  }
} else {
...
setInterval(async () => {
  try {
    // Only persist if MongoDB is connected to avoid 10s timeout lag
