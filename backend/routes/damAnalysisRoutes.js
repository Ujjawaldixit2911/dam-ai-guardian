...
router.post('/analyze', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    // Mock AI validation to check if it's actually a dam photo
    const originalName = req.file.originalname.toLowerCase();
    const invalidKeywords = ['cat', 'dog', 'person', 'selfie', 'car', 'wrong', 'fake'];
    const isInvalid = invalidKeywords.some(kw => originalName.includes(kw));
    
    // If it clearly looks like a wrong photo, reject it
    if (isInvalid) {
      console.log(`❌ Invalid photo detected: ${originalName}`);
      ...
      return res.status(400).json({
        success: false,
        error: 'Invalid Image',
        message: 'Please provide correct photo of dam'
      });
    }
